	...
	<body>
		...
		<div id="show-evaluation" class="content scaffold-show" role="main">
			...
			<ol class="property-list evaluation">
				...
				<li class="fieldcontain">
					<span id="evaluationType-label" class="property-label"><g:message code="evaluation.evaluationType.label" default="Evaluation Type" /></span>
					
						<span class="property-value"
                              aria-labelledby="evaluationType-label"><g:link
                                controller="evaluationType" action="show"
                                id="${evaluationInstance?.evaluationType?.id}">${evaluationInstance?.evaluationType?.name?.encodeAsHTML()}</g:link></span>
				...
				<li class="fieldcontain">
					<span id="evaluator-label" class="property-label"><g:message code="evaluation.evaluator.label" default="Evaluator" /></span>
						...
						<span class="property-value"
                              aria-labelledby="evaluator-label"><g:link
                                controller="member" action="show" id="${evaluationInstance?.evaluator?.id}">${evaluationInstance?.evaluator?.name?.encodeAsHTML()}</g:link></span>
					...
					<span id="speech-label" class="property-label"><g:message code="evaluation.speech.label" default="Speech" /></span>
						...
						<span class="property-value"
                              aria-labelledby="speech-label"><g:link
                                controller="speech" action="show" id="${evaluationInstance?.speech?.id}">${evaluationInstance?.speech?.speaker?.name?.encodeAsHTML()}</g:link></span>
