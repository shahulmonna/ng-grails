			...
			<thead>
					<tr>
                        <g:sortableColumn property="name" title="${message(code:
                                'club.name.label', default: 'Name')}" />

					
						<g:sortableColumn property="description" title="${message(code: 'club.number.label', default: 'Number')}" />
					
						<g:sortableColumn property="location" title="${message(code: 'club.location.label', default: 'Location')}" />
					

					

					
						<g:sortableColumn property="meetingTimings" title="${message(code: 'club.meetingTimings.label', default: 'Meeting Timings')}" />
				...
				<g:each in="${clubInstanceList}" status="i" var="clubInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td>${fieldValue(bean: clubInstance, field: "number")}</td>
					
						<td>${fieldValue(bean: clubInstance, field: "location")}</td>
					
						<td>${fieldValue(bean: clubInstance, field: "meetingTimings")}</td>
